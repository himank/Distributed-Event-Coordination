...
i) Insert request : This request from any client will enter new event ordering information to the server using the happened–before relationship. 
...
ii) Query request: This request from any client will ask the relative ordering between any two events. The queried events in this request will be separated with “white space” and will end 
with a semicolon.
...
Step1:
./decserver [-s] [-h] [-p port-number] [-l file]
...
Step2: Run Client by using
./decserver [-c ip of server] [-p port-number of server]
